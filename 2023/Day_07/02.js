...
const getHighCardValue = (card) => {
  const groups = cardValueGroup(card);
  const joker = groups.J ?? 0;
  delete groups.J;
  const groupsArray = Object.values(groups);
  const sortedGroupArray = groupsArray.sort((a, b) => b - a);
  // edge case where 'JJJJJ' card will give sortedGroupArray[0] undefined
  const highestGroup = (sortedGroupArray[0] ?? 0) + joker;
...
export const getTotalWinningFromCamelCardWithJoker = () => {
  ...
  console.log("Part two: total winnings from Cards with joker:", winnings);
};
